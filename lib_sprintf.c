...
void handleFlags(Token *token, char **buffer) {
  ...
  if ((token->Flags.PLUS_F && token->Spec != 'u')) {
    temp[0] = sign ? '-' : '+';
    lib_strcpy(temp + 1, sign ? (*buffer) + 1 : *buffer);
    lib_strcpy(*buffer, temp);
    sign = true;
  } else if ((token->Flags.SPACE_F && (*buffer)[0] != '-' &&
              token->Spec != 'u')) {
    temp[0] = ' ';
    lib_strcpy(temp + 1, *buffer);
    lib_strcpy(*buffer, temp);
    sign = true;
  } else {
    lib_strcpy(temp, *buffer);
  }
  lib_size_t size = lib_strlen(*buffer);
  if (token->Width > (int)size) {
    int diff = token->Width - size;
    if (token->Flags.MINUS_F) {
      lib_strcpy(temp, *buffer);
      lib_memset(temp + lib_strlen(temp), ' ', diff);
    } else {
      ...
      lib_memset(nulls ? sign ? temp + 1 : temp : temp, nulls ? '0' : ' ',
                 diff);
      lib_strcpy(nulls ? sign ? temp + diff + 1 : temp + diff : temp + diff,
                 nulls ? sign ? (*buffer) + 1 : *buffer : *buffer);
    }
  }
  if (lib_strlen(temp) != 0) {
    lib_strcpy((*buffer), temp);
  }
}
...
void handleSciAccurracy(char **buffer, int accuracy) {
  int digits = 0;
  lib_size_t len = lib_strlen(*buffer);
  int nullNotFound = 0;
  for (lib_size_t i = 0; i < lib_strlen(*buffer); i++) {
...
void parseHexSP(Token *token, char **buffer, va_list args) {
  ...
  for (int i = lib_strlen(*buffer) - 1; i >= 0; i--) {
    if ((*buffer)[i] != '0') {
      isNotNullsOnly = true;
    }
  }
...
void floatToBuffer(Token *token, long double num, char **buffer) {
  ...
  reverse(&bufferFloat);
  bufferInt = lib_strcat(bufferInt, bufferFloat);
  lib_strcpy(*buffer, bufferInt);
  free(bufferInt);
  free(bufferFloat);
}
...
void removeNulls(Token *token, char **buffer) {
  int len = lib_strlen(*buffer);
  char *dot = lib_NULL;
  dot = lib_strchr(*buffer, '.');
  if (dot) {
