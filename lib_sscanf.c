...
int lib_sscanf(const char *str, const char *format, ...) {
  ...
  if (!res_eof) {
    ...
    lib_size_t start = lib_strlen(str);
...
int parseStr(Token token, char **sptr, va_list args) {
  ...
  lib_size_t non_empty = lib_strcspn(*sptr, " \t");
...
int parseInt(Token token, char **sptr, long long *dest) {
  ...
  lib_size_t strLen = lib_strlen(*sptr);
...
int parseFloat(Token token, char **sptr, long double *dest) {
  ...
  if (isNANINF(sptr, negative, dest)) {
    ...
    lib_size_t numsCount = 0;
    *dest = lib_atof(sptr, negative, &token.width, &numsCount);
    if (numsCount == 0) {
      found = false;
    } else {
      found = true;
...
bool isNANINF(char **sptr, bool negative, long double *dest) {
  ...
  if (!lib_strncmp(lib_to_lower(*sptr), "nan", 3)) {
    *dest = NAN;
    found = true;
  } else if (!lib_strncmp(lib_to_lower(*sptr), "inf", 3)) {
    *dest = INFINITY;
    found = true;
  }
  if (found && negative) {
...
long long lib_atoi(char *sptr) {
  int res = 0;
  int digit = 0;
  for (lib_size_t i = 0; i < lib_strlen(sptr); i++) {
    digit = sptr[i] - '0';
    res = (10 * res) + digit;
  }
  return res;
...
long double lib_atof(char **sptr, bool negative, int *width,
                     lib_size_t *numsCount) {
  long double res = 0.0;
  bool dot = false;
